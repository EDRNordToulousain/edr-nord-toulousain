...
export default function HomePage() {
  return (
    <>
      ...
      <section className="py-20"><Container><SectionTitle kicker="Ils nous accompagnent" center>Nos partenaires</SectionTitle><p className="mx-auto mt-4 max-w-2xl text-center text-slate-600">Merci aux communes et entreprises locales qui soutiennent les projets de l’école de rugby.</p><div className="mt-10 grid gap-4 sm:grid-cols-3">{[...partners.mairies, ...partners.entreprises].map((partner) => <div key={partner} className="grid min-h-32 place-items-center rounded-2xl border border-dashed border-slate-300 bg-white p-5 text-center font-bold text-night"><span><span className="mb-2 block text-xs uppercase tracking-wider text-slate-400">Logo à venir</span>{partner}</span></div>)}</div><div className="mt-10 text-center"><Button href="/partenaires" variant="blue">Découvrir tous nos partenaires</Button></div></Container></section>
    </>
  );
}
