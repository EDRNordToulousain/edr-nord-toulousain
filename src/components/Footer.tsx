    ...
    <footer className="mt-20 bg-night text-white">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-14 md:grid-cols-3">
        <div className="flex gap-4">
          <Logo />
          <div><strong className="text-lg">{site.fullName}</strong><p className="mt-2 text-sm leading-6 text-white/65">{site.address.join(", ")}</p><a href={`mailto:${site.email}`} className="text-sm text-white/80 hover:text-white">{site.email}</a><p className="mt-3 text-xs text-white/55">SIRET : {site.siret}</p></div>
        </div>
